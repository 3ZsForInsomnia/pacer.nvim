...
function M.validate_state()
	...
	if M.active and not M.timer then
		...
		log.warn("State inconsistency detected - active=true but no timer. Resetting state.")
	...
	if M.active and M.bufnr and not vim.api.nvim_buf_is_valid(M.bufnr) then
		...
		log.warn("Active buffer (id=" .. M.bufnr .. ") is no longer valid. Stopping pacer.")
		M.reset_to_safe_state()
		return false
	end
...
function M.reset_to_safe_state()
	log.info("Resetting to safe state - cleaning up resources and state")
...
function M.cleanup()
	log.info("Full cleanup - clearing all resources and resetting state")
	M.reset_to_safe_state()
...
function M.adjust_wpm(change)
	...
	log.info("Speed changed from " .. old_wpm .. " WPM to " .. M.config.wpm .. " WPM", true)
end

function M.save_position()
	if not vim.api.nvim_buf_is_valid(vim.api.nvim_get_current_buf()) then
		log.error("Cannot save position - current buffer is invalid")
		return false
	end
	...
	log.info("Position saved - buffer=" .. bufnr .. " line=" .. (cursor[1] - 1) .. " col=" .. cursor[2])
	return true
end

function M.clear_position()
	log.info("Clearing saved positions")
	M.current_position = {
		bufnr = nil,
		line = 0,
		col = 0,
