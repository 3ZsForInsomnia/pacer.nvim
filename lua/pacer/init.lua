...
local function setup_autocommands()
	...
	vim.api.nvim_create_autocmd("VimLeavePre", {
		group = group,
		callback = function()
			print("Pacer: Neovim closing, cleaning up")
			local ok, err = pcall(function()
				require("pacer.core").stop()
				require("pacer.state").cleanup()
			end)
			if not ok then
				print("Pacer: Error during shutdown cleanup: " .. tostring(err))
			end
		end,
	...
	vim.api.nvim_create_autocmd("BufDelete", {
		...
		callback = function(ev)
			...
			if state.bufnr == ev.buf and state.active then
				print("Pacer: Active buffer deleted, stopping pacer")
				local ok, err = pcall(function()
					require("pacer.core").stop()
				end)
				if not ok then
					print("Pacer: Error stopping pacer on buffer delete: " .. tostring(err))
	...
	print("Pacer: Autocommands setup completed")
end

function M.setup(opts)
	print("Pacer: Initializing plugin")
	...
	if not ok then
		...
		print("Pacer: Setup failed: " .. tostring(err))
		return false
	end
	
	print("Pacer: Initialization completed successfully")
	return true
end
