...
function M.get_preset_config(preset_name)
	...
	if not preset then
		...
		print("Pacer: Preset '" .. preset_name .. "' not found. Available presets: " .. vim.inspect(vim.tbl_keys(presets)))
		return M.options
	end

	print("Pacer: Using preset '" .. preset_name .. "': " .. vim.inspect(preset))
	...
	if not ok then
		...
		print("Pacer: Error applying preset '" .. preset_name .. "': " .. tostring(preset_config))
		return M.options
	end
...
function M.setup(opts)
	...
	print("Pacer: Setting up with options: " .. vim.inspect(user_opts))
	...
	if not ok then
		...
		print("Pacer: Error merging user config with defaults: " .. tostring(merged_config) .. ". Using defaults.")
	...
	if M.options.wpm < 60 or M.options.wpm > 2000 then
		...
		print("Pacer: Invalid WPM in config (" .. M.options.wpm .. "), resetting to 300")
		M.options.wpm = 300
	end
	...
	if not M.options.highlight or not M.options.highlight.bg or not M.options.highlight.fg then
		...
		print("Pacer: Invalid highlight config, using defaults")
	...
	if not ok then
		print("Pacer: Error applying config to state: " .. tostring(err))
	end

	print("Pacer: Setup completed with final config: " .. vim.inspect(M.options))
	return M.options
end
...
function M.validate_environment()
	...
	if #issues > 0 then
		...
		print("Pacer: Environment validation found issues:")
		...
		print("Pacer: Environment validation passed")
	end
...
function M.ensure_setup()
	if not M._setup_called then
		print("Pacer: Lazy setup - initializing with defaults")
		...
		if not ok then
			print("Pacer: Error during lazy setup: " .. tostring(err))
		end
	end
end
